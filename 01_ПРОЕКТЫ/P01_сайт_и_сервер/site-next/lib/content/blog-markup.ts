...
export function renderBlogPostItem(post: BlogPost, index: number): string {
  ...
  return `<div class="t-feed__slider-grid__post-wrapper"><div class="t-feed__post t-slds__item js-feed-post t-align_left" data-slide-index="${index + 1}"><a href="${escapeHtml(href)}" class="t-feed__post-link" aria-label="Открыть статью: ${escapeHtml(post.title)}"><div class="t-feed__post-imgwrapper t-feed__post-imgwrapper_4-3"><div class="t-feed__post-bgimg t-bgimg" style="background-image:url('${escapeHtml(post.image)}');"></div></div><div class="t-feed__post-title t-name t-name_md">${escapeHtml(post.title)}</div><div class="t-feed__post-descr t-descr t-descr_xxs">${escapeHtml(post.excerpt)}</div><span class="eg-blog-card__open">Открыть статью <span aria-hidden="true">→</span></span></a></div></div>`;
}
